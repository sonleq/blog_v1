...
$sqlCategories = "SELECT n_category_id, v_category_title, v_category_meta_title, v_category_path FROM blog_category";
		...
		<div id="page-wrapper">
			<div id="page-inner">
				...
				if(isset($_REQUEST['addcategory'])){
					...
					}else if($_REQUEST['addcategory'] == "exists"){
						echo "<div class='alert alert-danger'><strong>Already Exists</strong> Changes to the Category was not added, there an existing category. </div>";
					}else if($_REQUEST['addcategory'] == "empty"){
						echo "<div class='alert alert-danger'><strong>One or more form field are empty!</strong> Please fill in the appropriate form field. </div>";
					}
							...
							<table class="table table-striped table-bordered table-hover">
								...
								<tbody>
									...
									while($rowCategories = mysqli_fetch_assoc($queryCategories)){
										...
										$id = (int)$rowCategories['n_category_id'];
										$name = htmlspecialchars($rowCategories['v_category_title'], ENT_QUOTES, 'UTF-8' );
										$metaTitle = htmlspecialchars($rowCategories['v_category_meta_title'],ENT_QUOTES, 'UTF-8');
										$categoryPath = htmlspecialchars($rowCategories['v_category_path'],ENT_QUOTES, 'UTF-8');
									...
									<tr>
										...
										<td>
											...
											<button class="popup-button" onclick="window.open('../categories.php?group=<?php echo urlencode($categoryPath); ?>', '_blank');">View</button>
